...
describeTelegramDispatch("dispatchTelegramMessage progress-lifecycle", () => {
  ...
  it("selects draft as sole commentary owner when progress.commentary is enabled", async () => {
    // Regression for GitHub #128038: without shouldDeliverCommentaryPayloads both
    // the progress draft and the durable payload path projected the same completed
    // preamble, producing duplicate 💬 lines in the progress message.
    const { answerDraftStream } = setupDraftStreams({ answerMessageId: 2001 });
    let capturedShouldDeliver: (() => boolean) | undefined;
    dispatchReplyWithBufferedBlockDispatcher.mockImplementation(
      async ({ dispatcherOptions, replyOptions }) => {
        // The owner callback must be present so the resolver can gate durable delivery.
        capturedShouldDeliver = replyOptions?.shouldDeliverCommentaryPayloads;
        expect(capturedShouldDeliver).toBeTypeOf("function");
        // At turn start the draft owns commentary: the durable path must be disabled.
        expect(capturedShouldDeliver?.()).toBe(false);
        await replyOptions?.onItemEvent?.({
          kind: "preamble",
          itemId: "c1",
          progressText: "Applying the change now.",
        });
        await replyOptions?.onItemEvent?.({
          kind: "preamble",
          itemId: "c2",
          progressText: "Publication is still running.",
        });
        await dispatcherOptions.deliver({ text: "Done" }, { kind: "final" });
        return { queuedFinal: true };
      },
    ...
    expectDeliveredReply(0, { text: "Done" });
    // Each preamble must appear at most once as a 💬 line across all window updates.
    const allWindowTexts = answerDraftStream.updatePreview.mock.calls.map(
      (call) => (call[0] as { text?: string }).text ?? "",
    );
    for (const text of allWindowTexts) {
      const duplicateCount = (text.match(/💬 Applying the change now\./g) ?? []).length;
