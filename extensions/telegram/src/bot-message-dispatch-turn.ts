      ...
      adapter: {
        ...
        resolveTurn: (): ChannelInboundTurnPlan<"provider_message_sending"> => ({
          ...
          replyOptions: {
            ...
            // When the progress draft renders commentary preambles as 💬 lines,
            // the draft owns them; the durable payload path must not also project
            // the same preamble. The callback selects one owner per turn via the
            // shared resolver (PR #121009), matching the Slack/Discord contract.
            shouldDeliverCommentaryPayloads:
              turn.progressPreambleEnabled && turn.commentaryProgressEnabled
